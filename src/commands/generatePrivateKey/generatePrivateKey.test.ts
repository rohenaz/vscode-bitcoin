...
describe('generatePrivateKey', () => {
  ...
  test('handles errors', async () => {
    ...
    try {
      await expect(generatePrivateKey(mockOutput, mockKeyVault)).rejects.toThrow('Test error');
      expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('Error generating private key: Error: Test error');
    } finally {
